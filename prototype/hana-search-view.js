(function(root){
  'use strict';
  const spots=[
    ['track','운동장 선',28,55,40,17,''],
    ['tree','운동장 나무',0,0,28,25,''],
    ['hedge','벤치 뒤 화단',23,75,13,14,''],
    ['fence','낮은 울타리',0,56,28,12,''],
  ...
  function mount(host,onDone,saved,onSave=()=>{},onSound=()=>{}){
    ...
    function render(lastAction){
      ...
      if(s.talk){
        ...
        if(portrait)host.insertAdjacentHTML('beforeend',`<div class="room-dialogue-portrait" aria-hidden="true"><img src="assets/${portrait}" alt=""></div>`);
