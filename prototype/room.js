(function(root){
 ...
 function mount(host,mode,onComplete,saved,onSave=()=>{}){
  ...
  function render(){
   ...
   host.innerHTML='<div class="room-toolbar"><div><span>1999 · '+(mode==='supplies'?'과학준비실':'방송실')+'</span><strong>'+goal+'</strong></div>'+btn('peek',peek?'표시 숨기기':'둘러보기')+'</div><div class="room-stage"><div class="room-picture"><img src="assets/'+art+'" alt="'+(mode==='supplies'?'실험 기구와 잠긴 서랍이 있는 과학준비실':'여러 촬영 소품과 비디오가 있는 방송실')+'">'+roomSpots.concat(scenery).map(([id,name,x,y,w,h])=>'<button type="button" class="room-hotspot '+(peek?'revealed':'')+'" data-spot="'+id+'" aria-label="'+name+' 조사" style="left:'+x+'%;top:'+y+'%;width:'+w+'%;height:'+h+'%"><svg class="room-object-glow" viewBox="'+x+' '+y+' '+w+' '+h+'" preserveAspectRatio="none" aria-hidden="true"><path d="'+(outlines[id]||scenery.find(item=>item[0]===id)?.[6])+'"/></svg><span>'+name+'</span></button>').join('')+'</div></div><div class="room-bag" aria-label="소지품">'+btn('bag','가방 열기')+'<strong>'+(s.selected==='key'?'사용 중: 작은 열쇠 → 잠긴 서랍을 누르세요':s.selected==='tape'?'사용 중: 테이프 → 비디오를 누르세요':'물건을 사용하려면 가방을 여세요')+'</strong></div><div class="room-speech" role="status"><b>'+speaker+'</b><p>'+line+'</p></div>';
