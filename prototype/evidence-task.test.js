...
assert.equal(E.initial({answers:valid,explanation:'설명',reviewed:true,complete:true}).complete,true,'v2 needs one core reason; transfer responses are optional teacher work');
assert.equal(E.initial({answers:valid,reviewed:true,complete:true}).complete,false,'a core reason or confirmed oral response is still needed');
